...
class User extends TableBase {
  User({this.id, this.name, this.email, this.password, this.isDeleted}) {
    _setDefaultValues();
    softDeleteActivated = true;
  }
  User.withFields(this.name, this.email, this.password, this.isDeleted) {
    _setDefaultValues();
  }
  User.withId(this.id, this.name, this.email, this.password, this.isDeleted) {
    _setDefaultValues();
  }
  // fromMap v2.0
  ...
  @override
  List<dynamic> toArgs() {
    return [name, email, password, isDeleted];
  }

  @override
  List<dynamic> toArgsWithIds() {
    return [id, name, email, password, isDeleted];
  }
  ...
  Future<int?> upsert({bool ignoreBatch = true}) async {
    try {
      final result = await _mnUser.rawInsert(
          'INSERT OR REPLACE INTO user (id, name, email, password,isDeleted)  VALUES (?,?,?,?,?)',
          [id, name, email, password, isDeleted],
          ignoreBatch);
      if (result! > 0) {
